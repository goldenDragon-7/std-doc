// Package lifecycle is the doc-archive layer behind `roll` and `graduate`. It
// owns Covenant V — "the palace is the archive":
//
...
// (shells to git via os/exec). Side-effecting inputs — the timestamp
...
// Slugify: lowercase, non-alphanumeric runs -> "-", trimmed; "doc" if empty.
...
// RollRecord is what a Roll did — the roll result.
...
// ShedRecord is what a Graduate shed — the graduate result.
...
func Graduate(publishedDir, version string, now NowFn) (*ShedRecord, error) {
	...
	// keys in order (version, at, comments, files).
