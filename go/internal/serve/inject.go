...
// the native Go inject + serve layer
// ZERO Python: no os/exec, no interpreter, ever.
package serve
...
// The exact tag forms inject writes, so a doc
...
// Note the two-space indent before continuation lines, so
...
// idempotently. Returns a human-readable status string.
...
// feedback/ path component (the "feedback" path component).
...
// the feedback dir exists. Printed output is human-readable.
