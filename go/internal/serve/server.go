...
// --strict-port). 33333 was chosen as the kingdom's std-doc serving floor.
const MinPort = 33333

// Options configures the serve command. Mirrors server.py's argparse defaults.
type Options struct {
...
// commonHeaders sets the no-cache + CORS headers server.py applies to every
...
func scanBind(start, scan int, strict bool) (net.Listener, int, error) {
	// Bind IPv4 explicitly (0.0.0.0), matching server.py's socketserver AF_INET.
...
// heartbeat writes watcher.json every interval, folding watch.py's presence job
...
// server.py's _watchdog. os.Exit because a dev server has no graceful-close upside.
