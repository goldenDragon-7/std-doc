...
> The lever that makes the Go port a **transcription**. A primitive's render
> logic is expressed as *structured data* interpreted by ONE small engine
> (`engine/core/template.py`). Port that interpreter once and all 24 primitives —
> being data — copy across unchanged. This is **not** a string mini-language and
> **not** a user-facing component system (both rejected in the PRD's locked
> decisions); it is a closed instruction set of dicts.
...
## Conversion status (P5)

**17 of 20 primitive types render from data** (render() delegates to the
interpreter), byte-identity locked by the conformance oracle: hero,
decisionmatrix, descent, crux, endpoint, callout, chip, compare, spectrum,
badges, threeaxes, children, ascii, mermaid, control, cardgrid, table.

The remaining 3 files stay Python **by design** — they are not HTML-templating
and a Go port reimplements them directly, not as template data:

| primitive | why it stays Python |
|---|---|
| `diagrams` (DiagramList) | a thin dispatcher — picks `mermaid`/`ascii` per item; not its own HTML |
| `evidence` | a core adapter: `render()` just calls `ctx["render_evidence"]` |
| `sections` | core numbering + anchor generation (`anchor(slug, j, k)`) — core-owned, not plugin content |
| `statetrack`, `swimlane` | D2 diagram engines — shell out to d2/mermaid and inline SVG; not text-templating |

So the format covers every pure HTML-structure primitive. The interpreter op set
that carries them is the whole transcription surface a port must reproduce.
...
The three shapes that prove expressiveness (`engine/tests/test_template.py`,
byte-identical to the Python `render()`):
...
## How a primitive uses a template (P5)

A converted primitive keeps its `type`, `css()`, `validate()`, `needs` and
delegates `render()` to the interpreter:

```python
from core.template import render_template
from primitives._templates import TEMPLATES

class Hero(RenderPrimitive):
    type = "hero"
    def render(self, block, ctx):
        return render_template(TEMPLATES["hero"], block, ctx)
```

The conformance oracle (`conformance/`) guarantees the bytes don't move when a
primitive is flipped to its template — that is the strangler proof for P5.
