...
There is a runnable fixer for the first three: **`engine/scripts/mermaid_fix.py`**
applies all of these transforms to an HTML or `.mmd` file (or stdin). Run it
before you ship, or just author correctly using the rules below.

```bash
python3 engine/scripts/mermaid_fix.py path/to/page.html            # fix in place (--write)
python3 engine/scripts/mermaid_fix.py page.html --check            # report only, exit 1 if changes needed
cat diagram.mmd | python3 engine/scripts/mermaid_fix.py -          # filter on stdin
```
...
**Fix — strip emoji from subgraph labels at embed time:**
...
def strip_emoji_subgraph(content):
    return re.sub(
        r'(subgraph\s+\w+\[")([^"]*?)(")',
        lambda m: m.group(1) + EMOJI_RE.sub('', m.group(2)).strip() + m.group(3),
        content)
```
...
**Fix — flatten `\n` to a space inside edge labels at embed time:**

```python
def fix_edge_labels(content):
    return re.sub(
        r'\|([^|\n]+?)\|',
...
The `[^|\n]` avoids matching across real newlines, preventing false positives on
multiline diagram source.
...
**Fix A — convert Bootstrap classDefs to dark-mode equivalents** (the fixer does
this; the mapping is the source of truth in `mermaid_fix.py`):
...
## Bug 4 (process) — `inject.py` is wiped by full HTML rewrites

The feedback widget tags live *in* the HTML. If you regenerate a page from
scratch (a full `index.html` rewrite, a fresh publish), those tags are gone and
the page silently stops listening for comments. **Re-run `inject.py <dir>` after
every full rewrite.** (The v2 `publish.py` already re-wires the widget on every
run; this bites hand-rewrites.)
...
- **Assets:** `engine/lib/mermaid.css` + `engine/lib/mermaid.js`, served at
  `/lib/mermaid.{css,js}` by BOTH the std-doc server and the in-lion Library (the
  lion's `/lib/{fname}` is a single traversal-safe passthrough over `engine/lib/`
  — no per-asset allow-list).
- **Wiring:** `inject.py` detects `class="mermaid"` on a page and auto-adds the two
  CDNs (mermaid@11 + svg-pan-zoom) + the mermaid.css/js tags. Pages with no diagram
  get nothing (zero CDN cost). `publish.py` emits the card for a source.json
  `diagrams[].mermaid` node, then inject wires it.
...
must be self-contained (zero EXTERNAL refs — see `freeze.py`), and mermaid renders
via JS, so **`freeze.py` bakes each `.mermaid` block to static inline `<svg>` once**
...
### Dependency (served mode; inject.py adds these for you)
...
Living docs are dark. Mermaid assumes light. **Run `mermaid_fix.py --check`
before you ship, and open the page in a browser to confirm zero bomb icons** —
