...
re-colour a matrix cell, lock in a crux, redraw a diagram, add a clarifying line.
...
> A hand-placed `data-cf-change` is **wiped the next time `publish.py` re-bakes
...
6. **Commit, don't waffle.** When they answer a crux, *lock it in* — restyle the
