...
# Example: ./setup.sh examples/hello-living-doc        # serves on :5050, never idles out
...
#   1. injects the feedback layer into every *.html in <doc-dir>   (was inject.py)
#   2. serves the directory + the feedback inbox API               (was server.py)
#   3. writes the presence heartbeat so the page shows "Claude is   (was watch.py)
...
# free one if $PORT is busy — no more stomping on 5050/5051).
EXISTS=0
